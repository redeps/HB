...
server <- function(input, output, session) {
  ...
  observeEvent(input$file1, {
    inFile <- input$file1
    data <- read.csv(inFile$datapath)
    hopNotesDT$data <- data[c(2,34,35)] %>%
      subset(!is.na(Value))
  })
  ...
  observeEvent(input$saveOut, {
    ...
    chosenHops <- data.frame(
      ...
      approxbeta = newHopoptions()$data[[7]],
      cohumulone = newHopoptions()$data[[8]],
      MyrcenePER100G = newHopoptions()$data[[9]],
    ...
    chosenYeasts <- data.frame(
      ...
      Lab = newYeastoptions()[[2]],
      Attentuation = newYeastoptions()[[3]],
      Temperature = newYeastoptions()[[4]],
      Tolerance = newYeastoptions()[[5]],
      minatt = newYeastoptions()[[6]],
      maxatt = newYeastoptions()[[7]],
      avgatt = newYeastoptions()[[8]],
      mintemp = newYeastoptions()[[9]],
      maxtemp = newYeastoptions()[[10]],
      Amount = shinyValue('Yeastv2_', nrow(newYeastoptions())))
